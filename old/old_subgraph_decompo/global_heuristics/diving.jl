...
function solve_subgraph_decompo_1vn(instance, v_node_partitionning)
    ...
    master_problem_integer = set_up_master_problem_integer(instance, vn_decompo)
    for subgraph in vn_decompo.subgraphs
        for column in subgraph.columns
            add_column_integer(master_problem_integer, instance, subgraph, column)
        end
    end
   
    optimize!(master_problem_integer.model)
