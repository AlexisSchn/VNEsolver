...
function set_up_subpb(model, subinstance, original_instance, v_subgraph, s_network_original_nodes)
    ...
    for s_node in vertices(s_network)
        ...
        @constraint(model,  sum(y[v_edge, s_edge] for s_edge in get_in_edges(s_network_dir, s_node) for v_edge in edges(v_network))
                            ...
                            + sum(y_ghost[ghost_edge, s_edge] for s_edge in get_in_edges(s_network_dir, s_node) for ghost_edge in ghost_edges)
                            + sum(y_ghost[ghost_edge, s_edge] for s_edge in get_out_edges(s_network_dir, s_node) for ghost_edge in ghost_edges)
