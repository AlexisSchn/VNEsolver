...
function set_up_pricers_sn_decompo(instance, v_subgraph, path = "")

    s_network = instance.s_network
    v_network = instance.v_network

    sn_clusters = get_sn_decompo(instance, v_subgraph, path)

    pricers = []
    for (i_cluster, cluster) in enumerate(sn_clusters)
        sub_s_network = my_induced_subgraph(s_network, cluster, "sn_$i_cluster")
        subinstance = Instance_Undir_VNE_1s(v_subgraph.graph, sub_s_network)

        model = Model(CPLEX.Optimizer)
        set_up_subpb(model, subinstance)
        
        current_pricer = PricerSubSubstrate(cluster, v_subgraph, subinstance, instance, model)

        push!(pricers, current_pricer)
    end
...
function get_sn_decompo(instance, v_subgraph, path = "")
    ...
    mkpath(current_path) 
    savefig(p, joinpath(current_path, "base_partition_sn.png"))
    ...
    p = visu_partitionning(s_network, clusters)
    savefig(p, joinpath(current_path, "corrected_partition_sn.png"))
    ...
    for cluster in clusters
        ...
        p = visu_added_nodes(s_network.graph, cluster, added)
        savefig(p, joinpath(current_path, "subsn_$i_cluster.png"))
...
function solve_pricers_sn_decompo(pricer)
