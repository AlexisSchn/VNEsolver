...
struct SubProblemReduced
    model
    s_network
    v_network
    subgraph
    current_constraints #constraints to delete at each iter
end


function set_up_pricer_reduced(instance, subgraph)
    ...
    return SubProblemReduced(model, instance.s_network, instance.v_network, subgraph, []);
end


function update_solve_pricer_reduced(instance, vn_decompo, pricer, dual_costs)
    ...
    #---------- Additional simplifying constraints:
    
    #remove previous ones
    for con in pricer.current_constraints
        delete(model, con)
    end
    empty!(pricer.current_constraints)

    # add new ones
    for v_node in vertices(subgraph.graph)
        to_constrain = false

        prices_s_nodes = zeros(nv(s_network))
        #prices_s_nodes[s_node] = s_network[s_node][:cost] - dual_costs.capacity_s_node[s_node] # let's only listen to the the dual variables that interest us..?
        ...
        for connecting_edge in vn_decompo.v_edges_master
            if subgraph ∈ keys(vn_decompo.v_nodes_assignment[src(connecting_edge)])
                if v_node == vn_decompo.v_nodes_assignment[src(connecting_edge)][subgraph]
                    to_constrain = true
                    for s_node in vertices(s_network)
                        prices_s_nodes[s_node] += - dual_costs.flow_conservation[connecting_edge][s_node] - dual_costs.departure[connecting_edge][s_node]
                    end
                end
            end
            if subgraph ∈ keys(vn_decompo.v_nodes_assignment[dst(connecting_edge)])
                if v_node == vn_decompo.v_nodes_assignment[dst(connecting_edge)][subgraph]
                    to_constrain = true
                    for s_node in vertices(s_network)
                        prices_s_nodes[s_node]  += dual_costs.flow_conservation[connecting_edge][s_node]
                    end
                end
            end
        end
        ...
        if to_constrain
            sorted_s_nodes = sortperm(prices_s_nodes)
            new_cons = @constraint(model, sum(model[:x][v_node, s_node] for s_node in sorted_s_nodes[1:15]) == 1)
            push!(pricer.current_constraints, new_cons)
        end
    end
