...
function solve_subgraph_decompo_1vn(instance, v_node_partitionning)
    ...
    add_dummy_cols_better(instance, vn_decompo, master_problem)
...
function add_dummy_cols_better(instance, vn_decompo, master_problem)
    ...
    # Rank the substrate nodes by order of highest degree

    s_node_degrees = [(v, degree(s_network, v)) for v in vertices(s_network)]

    # Sort nodes by degree in descending order and take the top five
    s_nodes_degree_sorted = sort(s_node_degrees, by=x -> -x[2])

    println(s_nodes_degree_sorted)
    # For each subgraph, create a dummy column using those nodes, and add it to the master problem
    i_s_node = 1
    for subgraph in vn_decompo.subgraphs
        
        affectation_necessary = Dict()
        for v_node in vertices(subgraph.graph)
            v_node_in_original = subgraph.nodes_of_main_graph[v_node]
            additional_degree = degree(v_network)[v_node_in_original] - degree(subgraph.graph)[v_node]
            if additional_degree > 0
                affectation_necessary[v_node] = s_nodes_degree_sorted[i_s_node][1]
                i_s_node += 1
            end
        end
        
        println("Affection : $affectation_necessary")

        # Now let's add the column
        lambda = @variable(model, base_name = "λ_$(subgraph.graph[][:name])_dummy",lower_bound = 0., upper_bound = 1.0);
        #master_problem.lambdas[subgraph][column] = lambda
        set_objective_coefficient(model, lambda, 999999)
        ...
        for v_edge in master_problem.vn_decompo.v_edges_master
            ...
            if subgraph in keys(master_problem.vn_decompo.v_nodes_assignment[src(v_edge)])
                ...
                set_normalized_coefficient(
                    master_problem.model[:flow_conservation][v_edge, affectation_necessary[v_node_in_subgraph]], 
                    lambda, 
                    1)
        ...
        for v_edge in master_problem.vn_decompo.v_edges_master
            ...
            if subgraph in keys(master_problem.vn_decompo.v_nodes_assignment[src(v_edge)])
                ...
                set_normalized_coefficient(
                    model[:departure][v_edge, affectation_necessary[v_node_in_subgraph]], 
                    lambda, 
                    1 )
