...
function solve_subgraph_decompo_1vn(instance, v_node_partitionning)
    ...
    while keep_on && nb_iter < nb_iter_max
        ...
        total_subpb_obj = 0
        for subgraph in vn_decompo.subgraphs
            columns, obj_values = update_solve_pricer(instance, vn_decompo, pricers[subgraph], dual_costs)
            total_subpb_obj += obj_values[1]
            for i_sol in eachindex(columns)
                if obj_values[i_sol] < -0.0001
                    keep_on = true 
                    add_column(master_problem, instance, subgraph, columns[i_sol])
                    nb_columns += 1
                    push!(subgraph.columns, columns[i_sol])
                end
                #println("Valuefor col $(i_sol): $(obj_values[i_sol])")
                #println("Et la teté de la col $(columns[i_sol])")
            end
...
function set_up_pricer(instance, subgraph)
    ...
    #set_optimizer_attribute(model, "PoolSolutions", nb_sol)
...
function update_solve_pricer(instance, vn_decompo, pricer, dual_costs)
    ...
    for i in 1:nb_sol
        ...
        x_values = value.(model[:x]; result=i)
        y_values = value.(model[:y]; result=i)
        cost_of_column = 0.
