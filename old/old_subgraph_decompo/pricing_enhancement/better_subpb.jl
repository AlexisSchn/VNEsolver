...
function set_up_decompo(instance, node_partitionning)
    ...
    for (i_subgraph, v_nodes) in enumerate(node_partitionning)
        ...
        for (i_node, v_node) in enumerate(v_nodes)
            ...
            #println("Mhhh okay so in the original graph the degree is $(degree(vn)[v_node])), but in the subgraph it is $(degree(subgraph.graph)[i_node])")
            subgraph.additional_degree[i_node] = degree(vn)[v_node] - degree(subgraph.graph)[i_node]
        end
...
function set_up_pricer(instance, subgraph)
    ...
    # edge capacity and edge demands from the original graph: a fun thing !??
    for s_node in vertices(s_network)
        @constraint(model, 
            sum( subgraph.graph[src(v_edge), dst(v_edge)][:dem] * (y[v_edge, s_edge]) 
                for v_edge in edges(subgraph.graph) for s_edge in get_out_edges(s_network_dir, s_node)∪get_in_edges(s_network_dir, s_node))
            <=
            sum(s_network_dir[src(s_edge), dst(s_edge)][:cap] for s_edge in get_out_edges(s_network_dir, s_node))
                - sum( subgraph.additional_degree[v_node] * x[v_node, s_node] for v_node in vertices(subgraph.graph))
        )
    end
...
function get_initial_set_of_columns(instance, vn_decompo, master_problem)
    ...
    for subgraph in vn_decompo.subgraphs
        ...
        column = solve_integer_with_forbidden_nodes_and_force_placement(current_instance, used_nodes, forced_placement, subgraph)
...
function solve_integer_with_forbidden_nodes_and_force_placement(instance, forbidden_nodes, forced_placement, subgraph)
...
function get_initial_set_of_columns_better(instance, vn_decompo, master_problem)
    ...
    for subgraph in vn_decompo.subgraphs
        ...
        for group in groups
            ...
            # create the plne
            column = solve_integer_with_placement_restriction(current_instance, placement_restriction, subgraph)
            # add the column
            if column != false
                add_column(master_problem, instance, subgraph, column)
                push!(subgraph.columns, column);
...
# Todo: use the base function...
function solve_integer_with_placement_restriction(instance, placement_restriction, subgraph)
