...
function solve_subgraph_decompo_1vn(instance, v_node_partitionning)
    ...
    println("Decomposition set: ")
        println("For $v_network, there is: 
            ...
            $(length(vn_decompo.v_nodes_accross_subgraph)) nodes in several subgraphs,
            $(length(vn_decompo.v_edges_accross_subgraph)) edges accross several subgraphs,
            $(length(vn_decompo.v_nodes_master)) nodes in no subgraph,
            $(length(vn_decompo.v_edges_master)) edges in no subgraph")
        
    if length(vn_decompo.v_edges_accross_subgraph) > 0
...
function set_up_decompo(instance, node_partitionning)
    ...
    vn_decompo = NetworkDecomposition(subgraphs, node_assignment, v_node_master, v_nodes_in_several_subgraphs, v_edge_master, v_edges_accross_subgraph)
...
function set_up_master_problem(instance, vn_decompo)
    ...
    master_routing_costs = @expression(model, sum( s_network[src(s_edge), dst(s_edge)][:cost] * v_network[src(v_edge), dst(v_edge)][:dem] * y[v_edge, s_edge]
    ...
    @variable(model,
        0 <= x_splitted[
            v_node in vn_decompo.v_nodes_accross_subgraph,
            s_node in vertices(s_network)]
        <= 1)
    ...
    @constraint(
        ...
        splitting[
            v_node in vn_decompo.v_nodes_accross_subgraph, 
            s_node in vertices(s_network), 
            subgraph in keys(vn_decompo.v_nodes_assignment[v_node])],
        0
        ==
        x_splitted[v_node, s_node]
    )


    # Wait is that needed ? I kinda forgot..
    #= 
    @constraint(
        model,
...
function add_column(master_problem, instance, subgraph, column)
    ...
    for v_node in vertices(subgraph.graph)
        ...
        if v_node_in_original_graph in master_problem.vn_decompo.v_nodes_accross_subgraph
            set_normalized_coefficient(
                model[:splitting][v_node_in_original_graph, column.mapping.node_placement[v_node], subgraph], 
                lambda, 
                1)
...
function get_duals(instance, vn_decompo, master_problem)
    ...
    splitting = Dict()
    for v_node in vn_decompo.v_nodes_accross_subgraph
        splitting[v_node] = Dict()
        for s_node in vertices(s_network)
            splitting[v_node][s_node] = Dict()
            for subgraph in keys(vn_decompo.v_nodes_assignment[v_node])
                splitting[v_node][s_node][subgraph] = dual(model[:splitting][v_node, s_node, subgraph])
            end
        end
    end
...
function update_solve_pricer(instance, vn_decompo, pricer, dual_costs)
    ...
    placement_cost = @expression(model, 
        sum( ( s_network[s_node][:cost] - dual_costs.capacity_s_node[s_node] ) * subgraph.graph[v_node][:dem] * subgraph.nodes_cost_coeff[v_node] * model[:x][v_node, s_node] 
        ...
        for connecting_edge in vn_decompo.v_edges_master
            if subgraph ∈ keys(vn_decompo.v_nodes_assignment[src(connecting_edge)])
                ...
                add_to_expression!(
                    ...
                    -dual_costs.flow_conservation[connecting_edge][s_node] * subgraph.nodes_cost_coeff[v_node_subgraph], 
                    model[:x][vn_decompo.v_nodes_assignment[src(connecting_edge)][subgraph], s_node])
            end
            if subgraph ∈ keys(vn_decompo.v_nodes_assignment[dst(connecting_edge)])
                ...
                add_to_expression!(
                    flow_conservation_cost, 
                    +dual_costs.flow_conservation[connecting_edge][s_node] * subgraph.nodes_cost_coeff[v_node_subgraph], 
                    model[:x][vn_decompo.v_nodes_assignment[dst(connecting_edge)][subgraph], s_node])
            end
        end
    end
    ...
    for v_node in vn_decompo.v_nodes_accross_subgraph
        if subgraph in keys(vn_decompo.v_nodes_assignment[v_node])
            v_node_subgraph = vn_decompo.v_nodes_assignment[v_node][subgraph]
            for s_node in vertices(s_network)
                add_to_expression!(
                    splitting_costs,
                    -dual_costs.splitting[v_node][s_node][subgraph],
    ...
    for s_node in vertices(s_network)
        for connecting_edge in vn_decompo.v_edges_master
            if subgraph ∈ keys(vn_decompo.v_nodes_assignment[src(connecting_edge)])
                ...
                add_to_expression!(
                    departure_costs, 
                    -dual_costs.departure[connecting_edge][s_node] * subgraph.nodes_cost_coeff[v_node_subgraph], 
                    model[:x][v_node_subgraph,s_node])
            end
        end
    end
...
function get_initial_set_of_columns(instance, vn_decompo, master_problem)
    ...
    for subgraph in vn_decompo.subgraphs
        ...
        cost_coeff= subgraph.nodes_cost_coeff
        column = solve_integer_with_forbidden_nodes_and_force_placement(current_instance, cost_coeff, used_nodes, forced_placement)
...
function get_initial_set_of_columns_tkt(instance, vn_decompo, master_problem)
    ...
    for subgraph in vn_decompo.subgraphs
        ...
        if subgraph == vn_decompo.subgraphs[1]
            forced_placement[1] = 12
            forced_placement[2] = 13
            forced_placement[3] = 14
            forced_placement[4] = 19
        end

        if subgraph == vn_decompo.subgraphs[2]
            forced_placement[1] = 4
            forced_placement[2] = 3
            forced_placement[3] = 24
            forced_placement[4] = 25
        end

        if subgraph == vn_decompo.subgraphs[3]
            forced_placement[1] = 26
            forced_placement[2] = 21
            forced_placement[3] = 29
            forced_placement[4] = 28
        end

        if subgraph == vn_decompo.subgraphs[4]
            forced_placement[1] = 2
            forced_placement[2] = 1
            forced_placement[3] = 7
            forced_placement[4] = 8
        end
        =#
...
function solve_integer_with_forbidden_nodes_and_force_placement(instance, nodes_cost_coeff, forbidden_nodes, forced_placement)
    ...
    ### Objective
    placement_cost = @expression(model, sum( s_network[s_node][:cost] * v_network[v_node][:dem] * x[v_node, s_node] * nodes_cost_coeff[v_node]
