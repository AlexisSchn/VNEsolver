...
function solve_PSO(instance; nb_particle=25, nb_iter=50, time_max=5, print_things=true)
    ...
    function shortest_path_routing(v_node_placement) # TODO: do for real the thing where you remove the edges bro. It's important......
        ...
        for v_edge in edges(v_network)
            ...
            if is_still_original_s_network
                ...
                if path.src == 0
                    sp = a_star(s_network_dir, s_src, s_dst, distmx)
                    path = order_path(s_network_dir, sp, s_src, s_dst)
                    shortest_paths[s_src][s_dst] = path
                end
            else
    ...
    shortest_paths = [[Path() for i in 1:nv(s_network)] for j in 1:nv(s_network)]
