...
function solve_compact_pl_1vn(instance, time_solver = 30)
    ...
    for v_node in vertices(v_network)
        println("   Node $v_node:")
        for s_node in vertices(s_network_dir)
            if x_values[v_node, s_node] > 0.001 || x_values[v_node, s_node] < -0.001 
...
function solve_heuristic_relax(instance)
    ...
    while length(nodes_restricted) != length(vertices(v_network))
        ...
        # classement des noeuds (par ceux qui ont le plus de voisin, en %, déjà fait!)
        ratio = Dict()
        for v_node in vertices(v_network)
            if v_node ∉ nodes_restricted
        ...
        node_to_restrict = sorted_keys[1]
        println("Node to restrict : $node_to_restrict")
        for neighb in neighbors(v_network, node_to_restrict)
            nb_neighbors_done[neighb] += 1
        end
        push!(nodes_restricted, node_to_restrict)

        sorted_s_nodes = sort(collect(vertices(s_network_dir)), by=s_node -> -x_values[node_to_restrict, s_node])
        placement_restriction[node_to_restrict] = sorted_s_nodes[1:3]
        println("Placed on : $(placement_restriction[node_to_restrict])")
        x_values = value.(current_model[:x])
