...
function set_up_problem_ff_extended(instance, model)
    ...
    zizi_cost = @expression(model,  ne(v_network) * 10000 - 500 * sum(x[src(v_edge), s_node_1] + x[dst(v_edge), s_node_2] - z[v_edge, s_node_1, s_node_2] for v_edge in edges(v_network) for s_node_1 in vertices(s_network) for s_node_2 in vertices(s_network)))
    @objective(model, Min, placement_cost + routing_cost + zizi_cost);
        ...
        for s_node_1 in vertices(s_network)
            for s_node_2 in vertices(s_network)
                if s_node_1 != s_node_2
                    @constraint(model, z[v_edge, s_node_1, s_node_2] ≥ x[src(v_edge), s_node_1] - x[dst(v_edge), s_node_2])
                    @constraint(model, z[v_edge, s_node_1, s_node_2] ≥ x[dst(v_edge), s_node_2] - x[src(v_edge), s_node_1])
                end
            end
        end
    end
...
function solve_compact_ffpluskinda(instance; time_solver = 30, stay_silent=false, linear=false)
                        ...
                        if x_val_1 > 0.03
                            if x_val_2 > 0.03
                                ...
                                if value < min(x_val_1, x_val_2)-0.05
                                    ...
                                    @constraint(model, 2 * sum(model[:y][v_edge, s_edge] for s_edge in cut_s_edges)
                                        ≥ 
                                        model[:x][src(v_edge), s_node_1] + model[:x][dst(v_edge), s_node_2] - model[:z][v_edge, s_node_1, s_node_2])
