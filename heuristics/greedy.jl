...
function solve_greedy_better(instance)
...
function solve_multi_greedy(instance; number_greedy = 10, time_max = 10, better=true)
    ...
    while iter < number_greedy && time_overall < time_max
        ...
        if better
            result = solve_greedy_better(instance)
        else
            result = solve_greedy(instance)
        end
        if result[:mapping_cost] < best_cost 
            best_cost = result[:mapping_cost]
            best_mapping = result[:mapping]
            println("New best mapping found!  $best_cost")
        end
