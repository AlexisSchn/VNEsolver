...
function solve_greedy_pricer(v_subgraph, original_instance, vn_decompo, dual_costs; nb_iterations=500)
    ...
    function complete_partial_placement(partial_placement)
        ...
        for v_node in vertices(v_network)
            placement_cost += node_costs[placement[v_node]] * coeff_v_nodes[v_node] + additional_costs_dual[v_node][placement[v_node]]
        end
    ...
    coeff_v_nodes = [1. for i in 1:nv(v_subgraph.graph)]
    for v_node in vertices(v_subgraph.graph)
        v_node_original = v_subgraph.nodes_of_main_graph[v_node]
    ...
    for v_node in vertices(v_subgraph.graph)
        ...
        for cut_edge in vn_decompo.v_edges_master
            ...
            if original_v_node == src(cut_edge)
                for s_node in vertices(s_network)
                    current_additional_costs[s_node] -= dual_costs.flow_conservation[cut_edge][s_node] * coeff_v_nodes[v_node]
                    current_additional_costs[s_node] -= dual_costs.departure[cut_edge][s_node] * coeff_v_nodes[v_node]
                end
            end
        ...
        if original_v_node ∈ keys(vn_decompo.overlapping_nodes)
            #if v_subgraph in vn_decompo.overlapping_nodes[v_node_overlapping]
            for (i_subgraph_overlapping, subgraph_overlapping) in enumerate(vn_decompo.overlapping_nodes[original_v_node])
                if subgraph_overlapping == v_subgraph
                    for s_node in vertices(s_network)
                        current_additional_costs[s_node] += dual_costs.overlapping[s_node][original_v_node][i_subgraph_overlapping]
                        if i_subgraph_overlapping == length(vn_decompo.overlapping_nodes[original_v_node])
                            current_additional_costs[s_node] -= dual_costs.overlapping[s_node][original_v_node][1]
                        else
                            current_additional_costs[s_node] -= dual_costs.overlapping[s_node][original_v_node][i_subgraph_overlapping+1]
                        end
                    end
                end
            end
    ...
    while iter < nb_iterations
        # Construct initial mapping
        s_nodes_scores = [ (centrality_nodes[s_node]*0 + 0.5 * rand() ) for s_node in capacited_nodes ]
